...
// ============================================================
var LIC_PRODUCT_NAME = "AstroMaxClarity";
var LIC_SETTINGS_KEY = "/AstroMax/AstroMaxClarity/licenseKey";
...
function buildLum(srcWin, lumWin) {
   var src = srcWin.mainView.image;
   var w = src.width, h = src.height, np = w*h;
   var rect = new Rect(0,0,w,h);
   if (src.numberOfChannels === 1) { copyWin(lumWin, srcWin); return; }
   var bR = new Float32Array(np), bG = new Float32Array(np), bB = new Float32Array(np);
   src.getSamples(bR, rect, 0);
   src.getSamples(bG, rect, 1);
   src.getSamples(bB, rect, 2);
   var bL = new Float32Array(np);
   for (var i = 0; i < np; i++)
      bL[i] = 0.2126*bR[i] + 0.7152*bG[i] + 0.0722*bB[i];
   lumWin.mainView.beginProcess(0);
   lumWin.mainView.image.setSamples(bL, rect, 0);
   lumWin.mainView.endProcess();
}

function buildZone(lumWin, center, halfWidth) {
   var hw = Math.max(0.01, halfWidth);
   var img = lumWin.mainView.image;
   var w = img.width, h = img.height, np = w*h;
   var rect = new Rect(0,0,w,h);
   var buf = new Float32Array(np);
   img.getSamples(buf, rect, 0);
   for (var i = 0; i < np; i++) {
      var t = Math.max(0, Math.min(1, 1 - Math.abs(buf[i]-center)/hw));
      buf[i] = t*t*(3-2*t);  // smoothstep
   }
   lumWin.mainView.beginProcess(0);
   lumWin.mainView.image.setSamples(buf, rect, 0);
   lumWin.mainView.endProcess();
}

function scaleMask(maskWin, factor) {
   var img = maskWin.mainView.image;
   var w = img.width, h = img.height, np = w*h;
   var rect = new Rect(0,0,w,h);
   var buf = new Float32Array(np);
   img.getSamples(buf, rect, 0);
   for (var i = 0; i < np; i++) buf[i] = Math.min(1, buf[i]*factor);
   maskWin.mainView.beginProcess(0);
   maskWin.mainView.image.setSamples(buf, rect, 0);
...
function applyClarity(baseWin, blurWin, shpWin, lumWin, maskWin,
   ...
   // Pure JS blend: base +/- mask * strength * (base - blur)
   var bBase = baseWin.mainView.image;
   var bBlur = blurWin.mainView.image;
   var bMask = maskWin.mainView.image;
   var w = bBase.width, h = bBase.height, np = w*h;
   var rect = new Rect(0,0,w,h);
   var bufBase, bufBlur, bufMask = new Float32Array(np);
   bMask.getSamples(bufMask, rect, 0);
   var strVal = Math.abs(clarityVal)/100.0*2.0;
   var sign = (clarityVal > 0) ? 1 : -1;
   baseWin.mainView.beginProcess(0);
   for (var c = 0; c < bBase.numberOfChannels; c++) {
      bufBase = new Float32Array(np);
      bufBlur = new Float32Array(np);
      bBase.getSamples(bufBase, rect, c);
      bBlur.getSamples(bufBlur, rect, c);
      for (var i = 0; i < np; i++) {
         var detail = bufBase[i] - bufBlur[i];
         bufBase[i] = Math.min(1, Math.max(0, bufBase[i] + sign*bufMask[i]*strVal*detail));
      }
      baseWin.mainView.image.setSamples(bufBase, rect, c);
   }
...
function applyLumSharpening(baseWin,shpWin,lumWin,sigma,amount,threshold){
   ...
   // Pure JS luminance ratio: base * (lumSharp / lumOrig)
   buildLum(baseWin, lumWin);
   var lumOrig = getWin("_amc_lumo_", baseWin.mainView.image, true);
   copyWin(lumOrig, lumWin);
   buildLum(shpWin, lumWin);
   var bBase = baseWin.mainView.image;
   var bLO = lumOrig.mainView.image, bLS = lumWin.mainView.image;
   var w = bBase.width, h = bBase.height, np = w*h;
   var rect = new Rect(0,0,w,h);
   var bufLO = new Float32Array(np), bufLS = new Float32Array(np);
   bLO.getSamples(bufLO, rect, 0);
   bLS.getSamples(bufLS, rect, 0);
   baseWin.mainView.beginProcess(0);
   for (var c = 0; c < bBase.numberOfChannels; c++) {
      var buf = new Float32Array(np);
      bBase.getSamples(buf, rect, c);
      for (var i = 0; i < np; i++) {
         var ratio = Math.min(2.0, (bufLS[i]+1e-7)/(bufLO[i]+1e-7));
         buf[i] = Math.min(1, buf[i] * ratio);
      }
      baseWin.mainView.image.setSamples(buf, rect, c);
   }
   baseWin.mainView.endProcess();
}
...
function AstroMaxClarityDialog(){
   ...
   this.slWidthH  =mkSlider("Highlights zone width",   5,100, 50,1,"widthH"  );
   ...
   this.btnReset.onClick=function(){
      ...
      self.slClarityH.setValue(0);         self.slWidthH.setValue(50);
...
function main() {
   // Close any leftover _amc_ temp windows from a previous run
   // (handles both JS-context-reuse and a clean reset of the WIN pool)
   closeAllWins();
   ...
   for (var _i = _ws.length - 1; _i >= 0; _i--) {
      ...
      if (!_w.isNull && _w.mainView && !_w.mainView.isNull) {
         ...
         if (_id.indexOf("_amc_") === 0 || _id === "_AstroMaxClarity") { _w.forceClose(); }
      }
   }
   Console.hide();
   var lic = licCheck();
   ...
   if (dlg.srcView === undefined || dlg.srcView === null) {
      (new MessageBox("No open images found.\nPlease open an image first.",
         "AstroMaxClarity", StdIcon_Error)).execute();
      return;
   }
